...
path <- "/views/RapidCOVID-19virology-Public/Headlinesummary"

body <- read_html(httr::modify_url(host_url, 
                                   path = path, 
  ...
  for(index in result) {
    if (t$dataType == index["dataType"]){
      if (length(index$valueIndices) > 0) {
        ...
        frameNames[[name_index]] <- paste(index$fieldCaption, sep="-")
...
doses <- df$`SUM(F2)` %>% as.numeric()

wales <- data.frame(Region = "Wales") %>%
                      mutate("1st dose" = doses[2], #putting it all in a data frame
                             "2nd dose" = doses[1], 
                             "Total doses" = sum(doses), 
                             "Publication_date" = Sys.Date())

if(!file.exists('data/wales_vaccine.csv')){
  write.csv(wales, 'data/wales_vaccine.csv', row.names = FALSE) 
} else {
  # Later: append to existing CSV
  write.table(wales, 'data/wales_vaccine.csv',
